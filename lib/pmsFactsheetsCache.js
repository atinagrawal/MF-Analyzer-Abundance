...
export async function getFactsheetDataForStrategy(providerName, iaName) {
  ...
  // properName (e.g. Sundaram's SISOP -> "Sundaram India Secular
  // Opportunities Portfolio") is a static fact set by the provider's own
  // fetcher, not something Gemini extracts -- lives on the raw doc, not
  // inside .extracted, so it's threaded through separately here.
  return { ...best.extracted, strategyName: best.strategyName, period: best.period, sourceUrl: best.url, properName: best.properName ?? null };
}
