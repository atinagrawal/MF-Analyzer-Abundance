...
export function buildQuartileReport(holdings, screenerFunds) {
  ...
  holdings.forEach(h => {
    ...
    holdingsByCategory.get(category).push({ holding: h, effectiveCode: screenerRow ? screenerRow.code : null, matchedViaName });
  });
  ...
  const categories = [...holdingsByCategory.entries()].map(([category, entries]) => {
    ...
    const funds = entries.map(({ holding, effectiveCode, matchedViaName }) => ({
      ...holding,
      matchedViaName, // true when this fund's own plan has no return data -- quartiles below are its Regular-plan counterpart's
