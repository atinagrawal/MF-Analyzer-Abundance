...
function barRankingSvg(rows, { width = 460, barHeight = 18, gap = 10, labelWidth = 190 } = {}) {
  ...
  const maxLabelChars = Math.max(8, Math.floor(labelWidth / 6.0));
  ...
  const bars = rows.map((row, i) => {
    ...
    return `
      <text x="0" y="${y + barHeight - 4}" font-size="11.5" fill="#444">${esc(truncateLabel(row.name, maxLabelChars))}</text>
      ...
      <text x="${pctX.toFixed(1)}" y="${y + barHeight - 4}" font-size="11.5" font-weight="600" fill="#2e4d2e">${row.pct.toFixed(1)}%</text>`;
  });
...
function overlapHeatmapSvg(names, grid, { cell = 68, labelWidth = 190 } = {}) {
  const n = names.length;
  const width = labelWidth + n * cell;
  const headerH = 32;
  const height = headerH + n * cell;
  const rows = [];

  names.forEach((name, j) => {
    rows.push(`<text x="${labelWidth + j * cell + cell / 2}" y="20" font-size="12" font-weight="700" fill="#2e7d32" text-anchor="middle">${j + 1}</text>`);
  });

  grid.forEach((row, i) => {
    rows.push(`<text x="0" y="${headerH + i * cell + cell / 2 + 4}" font-size="11" fill="#333">${i + 1}. ${esc(truncateLabel(names[i], 28))}</text>`);
    row.forEach((v, j) => {
      const x = labelWidth + j * cell;
      const y = headerH + i * cell;
      const color = i === j ? '#c8e6c9' : heatColor(v);
      rows.push(`<rect x="${x + 2}" y="${y + 2}" width="${cell - 4}" height="${cell - 4}" rx="6" fill="${color}" />`);
...
const MCAP_LABELS = { large: 'Large Cap', mid: 'Mid Cap', small: 'Small Cap', unclassified: 'Others' };

function stackedBarSvg(rows, { width = 460, barHeight = 24, gap = 12, labelWidth = 190 } = {}) {
  ...
  const bars = rows.map((row, i) => {
    ...
    return `<rect x="${labelWidth}" y="${y}" width="${trackWidth}" height="${barHeight}" rx="4" fill="#eef3ee" />
      <text x="0" y="${y + barHeight / 2 + 4}" font-size="11.5" fill="#444">${esc(truncateLabel(row.name, 30))}</text>${segs.join('')}`;
  });
