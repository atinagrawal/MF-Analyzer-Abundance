...
function overlapHeatmapSvg(names, grid, { cell = 84, labelWidth = 150 } = {}) {
  ...
  // Column headers use plain numbers (1, 2, 3...) rather than truncating
  // each fund name to fit an ~84px cell -- a name cut to 8-10 characters
  // (e.g. "Bandhan S...") is often indistinguishable from a sibling fund in
  // the same AMC family. Row labels below are numbered the same way, so a
  // reader matches column N to row N (a standard convention for symmetric
  // matrices); the HTML table directly below this chart also lists every
  // fund's full, untruncated name for unambiguous reference.
  names.forEach((name, j) => {
    rows.push(`<text x="${labelWidth + j * cell + cell / 2}" y="${headerH / 2}" font-size="13" font-weight="700" fill="#2e7d32" text-anchor="middle">${j + 1}</text>`);
  });

  grid.forEach((row, i) => {
    rows.push(`<text x="6" y="${headerH + i * cell + cell / 2 + 4}" font-size="11" fill="#333">${i + 1}. ${esc(truncateLabel(names[i], 20))}</text>`);
    row.forEach((v, j) => {
