...
// scheme FAMILY name (e.g. "HDFC Flexi Cap Fund - Growth Plan" ->
// "hdfc flexi cap"), matching how scripts/sync_scheme_riskometer.js
// normalizes AMFI's own per-scheme riskometer data before keying
// data/amfi-scheme-risk.json by it. Kept as an independent copy of that
// script's own normalizeSchemeFamilyName (and of this route's neighbour,
// app/api/proposal-studio/holdings/route.js's cleanSearchTerm) rather than a
// shared import -- same script/ESM-module boundary reasoning as
// cleanSearchTerm's own header comment.
...
// riskMap here is data/amfi-scheme-risk.json's `risk` object -- AMFI's own
// OFFICIAL per-scheme riskometer rating, the authoritative source. Distinct
// from matchBenchmarkRisk above, which only infers risk from the fund's
// BENCHMARK's riskometer as a fallback when a fund isn't in this map yet
// (e.g. a very recently launched scheme not yet in AMFI's fund-performance
// gateway).
function matchOwnSchemeRisk(schemeName, riskMap) {
  if (!schemeName || !riskMap) return null;
  const key = normalizeSchemeFamilyName(schemeName);
  if (!key) return null;
  return riskMap[key] || null;
}

export {
  fetchPdfText, getRiskometerUrl, parseRiskometer, fetchRiskometer,
  matchBenchmarkRisk, normalizeBenchmarkName,
  matchOwnSchemeRisk, normalizeSchemeFamilyName,
};
