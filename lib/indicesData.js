...
export function getValuationStatus(name, pe) {
  ...
  if (!t || pe == null) return { label: 'N/A', color: 'var(--muted, #666)', fill: '#ccc', pct: 0, zone: 'Neutral', desc: 'No historical benchmark thresholds defined' };
  const pct = Math.min((pe / t.max) * 100, 100);
  if (pe < t.low)  return { label: 'Undervalued', color: '#1b5e20', fill: '#43a047', pct, zone: 'Undervalued', desc: `Trailing P/E below ${t.low}x historical threshold` };
  if (pe < t.high) return { label: 'Fair Value',  color: '#e65100', fill: '#fb8c00', pct, zone: 'Fair Value', desc: `Trailing P/E between ${t.low}x and ${t.high}x` };
  return { label: 'Overvalued', color: '#b71c1c', fill: '#e53935', pct, zone: 'Overvalued', desc: `Trailing P/E above ${t.high}x historical threshold` };
}
