...
export async function getUserPlan(userId) {
  ...
  if (row.plan !== 'pro' && row.plan !== 'trial') return 'free';
 ...
 * @returns {Promise<{ tier: 'free'|'annual'|'lifetime'|'trial', expiresAt: string|null }>}
 */
export async function getUserPlanDetail(userId) {
  ...
  if (row.plan === 'trial' && row.plan_expires_at && new Date(row.plan_expires_at) > new Date()) {
    return { tier: 'trial', expiresAt: row.plan_expires_at };
  }
