...
async function r2Put(key, jsonString) {
  const res = await client().fetch(endpointFor(key), {
    ...
    headers: {
      ...
      // Without this, large bodies (confirmed: a multi-year, multi-PAN CAS
      // upload's JSON, several MB) fail on Vercel's production runtime with
      // R2/S3's "411 MissingContentLength" -- Node's fetch doesn't always
      // infer Content-Length for a large string body the way it does for
      // small ones (reproducible only on Vercel's actual runtime, not
      // locally or on GitHub Actions -- an internal streaming-vs-buffered
      // body threshold difference between fetch implementations, not
      // something this app's code controls). aws4fetch's SigV4 signer
      // explicitly excludes content-length from the signed header set
      // (UNSIGNABLE_HEADERS), so setting it here doesn't affect or break
      // the signature -- it only affects what's sent on the wire.
