...
// A REVERSAL voids an earlier purchase attempt -- most commonly a SIP
// instalment that failed after being provisionally recorded (AMC wording
// like "ER04: Insufficient Balance") and was retried the same day. It
// refunds the debited amount and gives back the (never actually held)
// units, so it's symmetric to a sell for both cash-flow and unit-count
// purposes -- NOT a no-op to be skipped. Confirmed against a real CAS: every
// scheme with an unhandled REVERSAL failed the reconciliation check below
// by exactly the reversed units, and every scheme without one reconciled
// cleanly, so this was silently blanking XIRR for any fund with even one
// bounced-and-retried SIP instalment in its history -- not a rare edge case
// for a SIP running long enough to hit a low-balance month.
const REVERSAL_TYPE_RE = /REVERSAL/;
...
export function schemeCashFlows(scheme) {
  ...
  for (const txn of txns) {
    ...
    const isReversal  = !isBuy && !isSell && REVERSAL_TYPE_RE.test(type);
    if (!isBuy && !isSell && !isReversal) continue;
    ...
    if (isBuy) {
      ...
      // Sell and reversal both give back units and return cash — a
      // reversal's date/amount/units are already the voided purchase's
      // own figures, so this exactly cancels it out.
      flows.push({ t: date.getTime(), amt: Math.abs(amount) });
      netUnits -= Math.abs(units);
    }
  }
