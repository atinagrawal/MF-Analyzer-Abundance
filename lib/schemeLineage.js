...
function stitchSeries(current, pred) {
  ...
  if (!(gapDays >= 0 && gapDays <= 30 && isFinite(ratio) && ratio > 0.001 && ratio < 1000)) return null; // not a clean transfer
  const k = cFirst.nav / pLast.nav;
  const head = pred.filter((p) => p.t < cFirst.t).map((p) => ({ t: p.t, nav: p.nav * k }));
  if (!head.length) return null;
  return { series: [...head, ...current], spliceDate: cFirst.t, from: pred[0].t, ratio: k };
}
