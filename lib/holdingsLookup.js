...
export class RequestThrottler {
  ...
  async acquire() {
    ...
    if (elapsed < requiredSpacing) {
      await new Promise((r) => setTimeout(r, requiredSpacing - elapsed));
    }
    this.lastRequestTime = Date.now();
  }
}

const sharedDefaultThrottler = new RequestThrottler(500, 100);

async function fetchWithRetry(url, options = {}, retries = 2, delayMs = 600, throttler = null) {
  if (throttler && typeof throttler.acquire === 'function') {
...
async function resolveSearchId(amfiCode, schemeName, throttler = null) {
  ...
  const res = await fetchWithRetry(url, { headers: FETCH_HEADERS }, 2, 600, throttler);
...
async function fetchFresh(amfiCode, schemeName, throttler = null) {
  const aliasMatch = KNOWN_SLUG_ALIASES.find((a) => a.pattern.test(schemeName));
  if (aliasMatch) {
    for (const slug of aliasMatch.slugs) {
      try {
        const aliasRes = await fetchWithRetry(`https://groww.in/v1/api/data/mf/web/v1/scheme/search/${slug}`, { headers: FETCH_HEADERS }, 2, 600, throttler);
  ...
  const searchId = await resolveSearchId(amfiCode, schemeName, throttler);
  if (!searchId) return null;

  const detailRes = await fetchWithRetry(`https://groww.in/v1/api/data/mf/web/v1/scheme/search/${searchId}`, { headers: FETCH_HEADERS }, 2, 600, throttler);
 ...
 */
export async function getHoldingsData(amfiCode, schemeName, options = {}) {
  const throttler = options.throttler || (options.throttle ? sharedDefaultThrottler : null);
  ...
  const fetchPromise = fetchFresh(amfiCode, schemeName, throttler)
    .then(async (data) => {
      if (data) {
        memCache.set(amfiCode, { data, ts: Date.now() });
        if (options.awaitPut) {
          await blobPut(amfiCode, data);
        } else {
          blobPut(amfiCode, data); // fire-and-forget for all live callers
        }
      }
      return data;
    })
