...
async function main() {
  ...
  console.log(`Testing batched fetch for ${mfIds.length} AMCs (date: 01-Jun-2026)...`);

  const date = '01-Jun-2026';
  const responses = await mapConcurrent(mfIds, 6, (id) =>
    fetchWithRetry(`https://www.amfiindia.com/api/statewise-data?MF_ID=${id}&date=${date}`)
  );
  ...
  responses.forEach((resp, idx) => {
    if (!resp || !Array.isArray(resp.data) || !resp.data.length) {
      console.log(`Failed AMC id=${mfIds[idx]} (${amcs[idx]?.mfName})`);
      return;
    }
    amcSuccessCount++;
    for (const r of resp.data) {
      ...
      if (!stateMap.has(stName)) {
        stateMap.set(stName, { state: stName, total: 0, equity: 0, debt: 0, liquid: 0 });
      }
  ...
  console.log(`\nSuccessfully fetched ${amcSuccessCount} of ${mfIds.length} AMCs!`);
